...
- Bevor ein geplanter Zeitplan wirksam wird, zeigt UEM transparent `voraussichtlich aktiv: Netzladen 01:00–03:00`, `Entladesperre 00:45–03:15` und den Grund. Für jede Netzladeentscheidung zeigt es zusätzlich: geplante Energiemenge (kWh), erwartete Ladeleistung, erwartete Dauer, günstigen All-in-Preis, erwarteten späteren Vergleichspreis, PV-/Lastprognose, geplanten Ziel-SoC und die wirtschaftliche Differenz inklusive Speicherverlusten/Verschleiß. Beispiel: `Netzladen geplant: 01:00–03:00, 8,0 kWh bei 17 ct/kWh; verhindert voraussichtlichen Bezug um 39 ct/kWh in der Nacht; Ziel-SoC 62 %.` Falls keine Netzladung wirtschaftlich oder technisch zulässig ist, steht ebenso eindeutig `Keine Netzladung geplant` mit dem ausschlaggebenden Grund, etwa `PV-Prognose ausreichend`, `Preisvorteil nach Verlusten zu gering`, `Netzladefreigabe aus` oder `Geräteadapter unterstützt Netzladung nicht`. Manuelle Sperren und Zeitpläne behalten Vorrang; ohne sichere Netzladefähigkeit bzw. explizite Freigabe bleibt es eine Empfehlung.
