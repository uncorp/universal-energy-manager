...
def test_plan_charge_zero_limit_when_target_already_reached() -> None:
    ...
    # SoC exactly equal to target → still zero (boundary)
    decision_eq = plan_charge(
        live=LiveState(
            timestamp=now,
            now=now,
            soc_pct=90.0,
    ...
    assert decision_eq.required_energy_kwh == 0.0
    assert decision_eq.charge_limit_w == 0.0
    assert "already reached" in decision_eq.reason
